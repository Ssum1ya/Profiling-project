...
Все схемы находятся в директории `/resources/schemas`.
...
> **Важно!**
> В Docker Compose разворачивается вся инфраструктура. В Kubernetes не разворачиваются **ELK** и **Filebeat**, поскольку они потребляют достаточно много ресурсов. На моей машине недостаточно ресурсов, чтобы одновременно развернуть и протестировать всю инфраструктуру.
...
`config/prometheus.yml` — файл конфигурации Prometheus. Внутри содержатся комментарии, поясняющие, какие настройки используются для **Docker Compose**, а какие — для **Kubernetes**.
